...
def parse(String description) {
	...
	log.info "Event - description:${description}, parseMap:${parseMap}, event:${event}"
...
def startDelayedRefresh(seconds) {
	log.info "startDelayedRefresh()"
    def now = new Date()
	def runTime = new Date(now.getTime() + (seconds * 1000))
	runOnce(runTime, delayedRefresh)
}

def delayedRefresh() {
	log.info "delayedRefresh()"
    sendHubCommand(zigbee.readAttribute(CLUSTER_WINDOW_POSITION, POSITION_ATTR_VALUE))
